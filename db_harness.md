...
서비스 JWT Access Token을 재발급하기 위한 Refresh Token 정보를 저장한다. 원본 Refresh Token은 저장하지 않고 해시만 저장한다.

| 컬럼 | 타입 | NULL | 의미 |
|---|---|---:|---|
| `refresh_token_id` | `BIGINT` | N | PK, AUTO_INCREMENT |
| `user_id` | `BIGINT` | N | 논리적 FK → `user.user_id` |
| `token_hash` | `VARCHAR(64)` | N | UNIQUE, Refresh Token 해시 |
| `expires_at` | `DATETIME(6)` | N | 만료 시각 |
| `created_at` | `DATETIME(6)` | N | 발급 시각 |
| `revoked_at` | `DATETIME(6)` | Y | 폐기 시각, NULL이면 미폐기 상태 |

인덱스:

- `PRIMARY KEY (refresh_token_id)`
- `UNIQUE (token_hash)`
- `INDEX (user_id, revoked_at, expires_at)`: 사용 가능한 회원 토큰 조회

서비스 JWT Access Token 자체는 DB에 저장하지 않고 HttpOnly Cookie로 전달한다.
...
Google OAuth 성공
  ...
  → JWT Access Token은 HttpOnly Cookie에 저장
  → Refresh Token 원본은 Cookie, 해시는 refresh_tokens에 저장
